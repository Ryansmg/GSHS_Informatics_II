...
int main()
{
  ...
  //가장 긴 재생시간을 파일의 최종 크기로 설정, Subchunk2Size에 반영
  headt.Subchunk2Size=headt1.Subchunk2Size;
  if(headt2.Subchunk2Size>headt.Subchunk2Size)
    headt.Subchunk2Size=headt2.Subchunk2Size;
  if(headt3.Subchunk2Size>headt.Subchunk2Size)
    headt.Subchunk2Size=headt3.Subchunk2Size;
    
  headt.ChunkSize=headt.Subchunk2Size+36; //ChunkSize에 반영
  
  head=writeheader(headt);             //값을 헤더 데이터로 변환
  ...
  for(int i=0; i<n; i++)
  {
    ...
    {
      int t=0;
      if(i<=(headt1.Subchunk2Size)/headt1.BlockAlign)
      {
        fread(buffer, size, 1, fp1); t+=ler(buffer, size);
      }
      if(i<=(headt2.Subchunk2Size)/headt2.BlockAlign)
      {
        fread(buffer, size, 1, fp2); t+=ler(buffer, size);
      }
      if(i<=(headt3.Subchunk2Size)/headt3.BlockAlign)
      {
        fread(buffer, size, 1, fp3); t+=ler(buffer, size);
      }
      lew(buffer, t, size);
